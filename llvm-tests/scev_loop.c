// RUN: %clang -O0 -Xclang -disable-O0-optnone -emit-llvm -S %s -o - | %opt -load-pass-plugin=%shlibdir/IOOpt%shlibext -passes="mem2reg,instcombine,loop-simplify,loop-rotate,indvars,io-opt" -S | %FileCheck %s
